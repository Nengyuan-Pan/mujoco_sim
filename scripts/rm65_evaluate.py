"""RM-65 双臂机器人 MPC+iLQT 网球击打评估脚本。

在 rm65_mpc_ilqr_5_5.py 基础上新增评估功能：
- 改善 MuJoCo 查看器光照（三灯布光）
- 球速分析（击打前后、反弹比）
- 球拍中心速度分析（峰值、时间序列）
- 关节限位分析（轨迹 vs joint range、最小裕度）
- 关节速度分析（峰值速度、时间序列）
- matplotlib 评估报告绘图

用法：
  python scripts/rm65_evaluate.py --viewer --seed 42
  python scripts/rm65_evaluate.py --seed 0 --no-plot      # 不绘图
  python scripts/rm65_evaluate.py --bounce --normal-flip --viewer  # 反弹球
"""
...
# RM-65 额定关节速度（rad/s），来自 datasheet
RM65_JOINT_VEL_LIMIT = np.array([3.14, 3.14, 3.14, 3.93, 3.93, 3.93])
...
def plot_evaluation(eval_data: dict, save_path: str) -> None:
    ...
    """
    if plt is None:
        logger.warning("matplotlib 未安装，跳过评估图绘制")
        return
