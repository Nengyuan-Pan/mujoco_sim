"""RM-65 纯 Python iLQR 快速版（性能对标脚本）。

在 rm65_mpc_ilqr_5_5.py 基础上，仅用 Python 实现快速版优化参数：
  replan_interval=20, first_plan_iters=15, horizon cap=40
  首次线搜索，其余跳搜索

用途：与 C++ 加速版 rm65_mpc_fast.py 做性能对比。
用法：
  python scripts/rm65_mpc_ilqr_5_7_python.py --seed 42 --normal-flip --viewer
"""
...
def main() -> None:
    ...
    parser.add_argument("--replan-interval", type=int, default=20, help="重规划间隔步数（默认20）")
