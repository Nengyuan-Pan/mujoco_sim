...
class TubeHittingCostWrapper:
    ...
    def terminal_cost(self, x: np.ndarray) -> float:
        """终端代价 = (1 - tube_ratio) * 全约束终端代价。

        使用完整 Q_p 矩阵（不做软投影），确保末端精确到达击打点。
        Tube 的走廊引导由 running_cost 中的 tube 代价提供。
        """
    ...
    def _compute_tube_derivatives_at_k(
        ...
        # ---- 1. 垂直偏离代价（连续惩罚，无 hinge loss 门槛）----
        # cost = Q_p * ||P_perp @ (p_ee - p_ref)||^2
        # grad: Q_p * 2 * J_p^T @ P_perp @ dp_perp, Hessian: Q_p * J_p^T @ P_perp @ J_p
        dp = p_ee - self._p_ball_ref
        dp_perp = self._P_perp @ dp
        l_x_tube[:n_q] += self._Q_p_tube * (J_p.T @ dp_perp)
        l_xx_tube[:n_q, :n_q] += self._Q_p_tube * (J_p.T @ self._P_perp @ J_p)
