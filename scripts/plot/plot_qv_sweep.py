...
OUT_DIR = ROOT / "results" / "exp_qv_sweep_20260603"
OUT_DIR.mkdir(parents=True, exist_ok=True)

# 实验数据
conditions = ["baseline", "Q_v×4", "Q_v_near×2", "Q_p降低", "Q_v×4+Q_p降低"]
hit_counts = [43, 40, 43, 41, 40]
total = 47
hit_rates = [c / total * 100 for c in hit_counts]
avg_v = [1.332, 1.380, 1.334, 1.389, 1.402]
std_v = [0.381, 0.316, 0.381, 0.342, 0.360]
min_v = [0.362, 0.504, 0.362, 0.320, 0.399]
max_v = [2.104, 2.332, 2.104, 2.058, 2.331]
avg_pos = [0.0476, 0.0443, 0.0476, 0.0575, 0.0573]
...
ax1.set_title("Q_v/Q_p 调参实验: 命中率 vs 球拍速度 (50 seeds, ball_speed=7m/s)", fontsize=14, pad=15)
fig.tight_layout()
fig.savefig(OUT_DIR / "qv_sweep_hitrate_speed.png", dpi=150, bbox_inches="tight")
plt.close()

# 图2: 球拍速度箱线图（用 min/avg/max 模拟）
fig2, ax3 = plt.subplots(figsize=(10, 6))
# 用 min, avg-std, avg, avg+std, max 做 boxplot
box_data = []
for i in range(len(conditions)):
    ...
    box_data.append({
        "med": med, "q1": q1, "q3": q3,
        "whislo": min_v[i], "whishi": max_v[i], "label": conditions[i]
    })

positions = np.arange(len(conditions))
bp = ax3.bxp(box_data, positions=positions, showfliers=False, patch_artist=True)
...
ax3.set_title("Q_v/Q_p 调参实验: 球拍速度分布 (min/avg±std/max)", fontsize=14, pad=15)
ax3.legend(fontsize=10)
ax3.tick_params(axis="y", labelsize=11)
fig2.tight_layout()
fig2.savefig(OUT_DIR / "qv_sweep_speed_boxplot.png", dpi=150, bbox_inches="tight")
plt.close()

# 图3: 位置误差 vs 球拍速度 散点（帕累托前沿）
...
for i, cond in enumerate(conditions):
    ...
    ax4.annotate(cond, (avg_pos[i], avg_v[i]), textcoords="offset points",
                 xytext=(10, 5), fontsize=10)

ax4.set_xlabel("命中 pos_error avg (m) — 越小越好 →", fontsize=12)
ax4.set_ylabel("击球瞬间球拍速度 avg (m/s) — 越大越好 ↑", fontsize=12)
ax4.set_title("Q_v/Q_p 调参: 位置精度 vs 球拍速度 (帕累托分析)", fontsize=14, pad=15)
ax4.invert_xaxis()
ax4.legend(fontsize=10)
ax4.grid(True, alpha=0.3)
ax4.tick_params(labelsize=11)
fig3.tight_layout()
fig3.savefig(OUT_DIR / "qv_sweep_pareto.png", dpi=150, bbox_inches="tight")
plt.close()

# 保存数据 CSV
csv_path = OUT_DIR / "qv_sweep_results.csv"
...
print(f"  qv_sweep_hitrate_speed.png  — 命中率+球拍速度双轴柱状图")
print(f"  qv_sweep_speed_boxplot.png  — 球拍速度分布箱线图")
print(f"  qv_sweep_pareto.png         — 帕累托分析散点图")
print(f"  qv_sweep_results.csv        — 原始数据")
