"""RM-65 V7: 终端=击球点（偏移=0）+ 满秩 Q_v + 来球反方向 + TCP/关节强约束。

=== V7 与 V6 的唯一差异 ===
  - 终端偏移 = 0（p_terminal = p_hit，击球步就是终端步，无额外偏移）
  - 默认开启 TCP/关节硬约束（terminal_exempt_steps=0）

=== V7 架构 ===
  1. 终端目标 = p_hit（击球点本身，无偏移）
  2. 终端速度 = 1.8 m/s * d_follow
...
=== 版本架构对比 ===
  ...
  版本         击球方向    终端目标          Q_v         Midpoint  随挥   硬约束
  v3           固定YAML    p_hit+0.01m      满秩[400]   无        无     是       96%
  v6           来球反向    p_hit+偏移       满秩[400]   无        PD    否       91%
  v7(本版本)   来球反向    p_hit(偏移=0)    满秩[400]   无        PD    是       ?

用法:
  python scripts/rm65_mpc_v7.py --serve-box --ball-speed 7 --viewer
  python scripts/rm65_mpc_v7.py --serve-box --ball-speed 7 --no-backswing
"""
...
def do_replan(
    ...
    cost_fn_plan = HittingCost(
        ...
        n_des=state.current_n_des if np.linalg.norm(state.current_n_des) > 1e-6 else None,
    )
...
def main() -> None:
    ...
    parser.add_argument("--max-tcp", type=float, default=1.8,
                        help="TCP 线速度硬限制 (m/s), v7 默认=1.8")
    parser.add_argument("--terminal-exempt-steps", type=int, default=0,
                        help="终段 qdot/TCP 豁免步数, v7 默认=0（全程硬限）")
    args = parser.parse_args()

    use_tube = True  # v6: 始终启用 tube（softmin 由 HittingCost 内置）
    ...
    # v6: 来球反方向作为击球方向（物理上更合理，拍面正对来球）
    d_follow = -v_ball_hit / (np.linalg.norm(v_ball_hit) + 1e-8)
    d_hat = d_follow

    # v6: 终端速度 = TCP 限速（可达目标），不再用不可达的 5.0 m/s
    v_hit_at_contact = 1.8 * d_follow  # 恰好等于 TCP 上限

    # v6: 随挥管道参数
    follow_through_length = 0.0  # v7: 终端偏移=0（击球步=终端步）
    ...
    # v6: 不设 midpoint（与 v2 一致，避免梯度冲突）
    # v7: 不在 base_cost 上设 softmin（由 TubeHittingCostWrapper 提供）

    # v7: 创建 Tube 代价包装器（v2 风格 softmin，per-candidate v/n）
    tube_cost_fn: TubeHittingCostWrapper | None = None
    if use_tube and hitting_tube is not None and not args.no_softmin:
        tube_cost_fn = TubeHittingCostWrapper(
            env, base_cost_fn, hitting_tube, k_hit_total, tube_cfg,
        )
        cost_fn = tube_cost_fn
        logger.info("[v6] TubeHittingCostWrapper 已创建（v2风格 softmin）")
    else:
        cost_fn = base_cost_fn
        logger.info("[v6] 使用标准 HittingCost（无 softmin）")
    ...
    for step in range(total_horizon):
        ...
        if need_replan:
            ...
            if hit_info_new is not None:
                ...
                # v7: 更新 Tube + TubeHittingCostWrapper（v2 风格 softmin）
                if use_tube:
                    hit_window = search_hit_window(
                    ...
                    if hit_window is not None and not args.no_softmin:
                        hitting_tube = build_hitting_tube(
                            hit_window, racket_speed, d_follow, tube_cfg,
                        )
