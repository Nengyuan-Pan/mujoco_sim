"""RM-65 双臂机器人 MPC+iLQT 网球击打脚本（后摆版本）。

在 rm65_mpc_ilqt.py 基础上新增：
- 关节1 后摆→前挥 Warm-start（A 方案）
- 关节空间运行代价保护后摆方向（B 方案）

用法：
  python scripts/rm65_mpc_backswing.py                  # 默认参数
  python scripts/rm65_mpc_backswing.py --viewer          # + 真实速度回放
  python scripts/rm65_mpc_backswing.py --seed 42         # 指定随机种子
  python scripts/rm65_mpc_backswing.py --backswing 0.6   # 自定义后摆幅度 (rad)
  python scripts/rm65_mpc_backswing.py --bs-ratio 0.4    # 自定义后摆占比
  python scripts/rm65_mpc_backswing.py --r-decay 0.5     # 自定义 R 退火段占比
"""
