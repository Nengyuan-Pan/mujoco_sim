"""连续运行 20 次 v8 击打仿真并保存 MP4 视频（单进程内循环）。
...
# 从 v8 导入全部辅助函数
from scripts.rm65_mpc_v8 import (
    fix_joint5_control,
...
logger = logging.getLogger("20hits_v8")

# ===== 参数 =====
parser = argparse.ArgumentParser(description="V8 连续 20 次击打 + MP4 视频")
...
output_dir = Path(args.output_dir) if args.output_dir else Path("results/20hits_v8_speed%d" % int(args.ball_speed))
...
def run_batch() -> None:
    ...
    if args.serve_box:
        fixed_horizon = 120
        total_horizon = 250
        serve_dist = args.serve_distance if args.serve_distance != 8.0 else SERVE_DIST_MAP.get(int(args.ball_speed), 9.5)
        logger.info(f"serve-box: distance={serve_dist}m")
    ...
    backswing_offset = -0.6
    backswing_ratio = 0.35
    use_r_decay = True
    r_decay_ratio = 0.40
    follow_through_steps = 40
    follow_through_length = 0.01
    ...
    if do_render:
        ...
        video_path = output_dir / "20hits_v8_continuous.mp4"
    ...
    for i in range(args.n_runs):
        ...
        if hit_info is None:
            logger.warning("[%2d/%d] seed=%d 球不可达，跳过" % (i + 1, args.n_runs, seed))
            results.append({"seed": seed, "hit": False, "pos_error": 999.0, "hit_type": "unreachable"})
            continue
        ...
        if min_margin_deg < 3.0:
            ...
            for dk in range(-search_range, search_range + 1):
                ...
                if not (dist_alt < workspace_radius and p_alt[2] > 0.3 and -0.60 < dz_alt < 0.55):
                    continue
        ...
        if hit_window is not None:
            hitting_tube = build_hitting_tube(hit_window, racket_speed, d_follow, tube_cfg)
        else:
            use_tube = False
        ...
        base_cost_fn = HittingCost(
            ...
            Q_p_running=0.0,
            R_joint_scale={0: 0.3} if use_backswing else None,
            q_des_traj=None, Q_joint=None, R_schedule=None,
            Q_n=500000.0, n_des=n_des_single,
        ...
        env_plan = AsyncReplanner(env, do_replan, replan_cfg, state=replan_state, model_path=model_path)
        _ = env_plan._ensure_env_plan()

        first_result = do_replan(first_request, env_plan.env_plan, replan_state, replan_cfg)
        ...
        hit_lock_active = False
        last_p_hit_check: np.ndarray | None = None
        ...
        for step in range(effective_total):
            ...
            if need_replan:
                remaining_horizon = effective_total - step
                ...
                if hit_info_new is not None:
                    ...
                    if use_tube:
                        hw = search_hit_window(
                            ...
                            remaining_horizon, tube_cfg, ball_direction="y",
                            current_step=0, robot_limits=robot_limits,
                            init_q=x_current[:NQ].copy(),
                        )
                        if hw is not None:
                            ht = build_hitting_tube(hw, racket_speed, d_follow, tube_cfg)
                            cost_fn = TubeHittingCostWrapper(
                    ...
                    if k_hit_new > far_threshold and not is_first_plan:
                        ...
                        U_buffer = u_jt
                        buffer_idx = 0
                        U_prev = U_prev if len(U_prev) > 0 else np.zeros((0, NU))
                        iters = 0
                        continue
                    ...
                    if hasattr(cost_fn, 'set_smoothness_scale'):
                        cost_fn.set_smoothness_scale(
                            float(s["Q_qdot_mult"]), float(s["Q_qddot_mult"]), float(s["Q_du_mult"]),
                        )

                    if use_r_decay:
                        R_schedule_new = compute_r_schedule(horizon_full, R, decay_ratio=r_decay_ratio)[:horizon_plan]
                    ...
                    if use_backswing and horizon_full > 0:
                        q_hit_ik = env.solve_ik(p_hit_new, q_init=x_current[:NQ], max_iter=150, eps=1e-3)
                        env.set_arm_state(np.concatenate([q_hit_ik, np.zeros(NQ)]))
                        ...
                        q_des_full[:, 0] = compute_joint1_backswing_trajectory(
                            x_current[0], x_current[NQ], q_hit_ik[0], qdot_hit_new[0],
                            horizon_full,
                            backswing_offset=cur_backswing_offset * bs_scale_plan,
                            backswing_ratio=backswing_ratio,
                        )
                        for j in range(1, NQ):
                            q_des_full[:, j] = np.linspace(x_current[j], q_hit_ik[j], horizon_full)
                        cost_fn.set_q_des_traj(q_des_full[:horizon_plan], Q_joint=None)
                    ...
                    if len(U_prev) >= horizon_full // 3:
                        ...
                        U_warm_full, _ = generate_backswing_warm_start(
                            ...
                            cur_backswing_offset, backswing_ratio, fix_joint5_angle, n_des_new,
                        )
            ...
            if fix_joint5_angle is not None:
                u_cmd = fix_joint5_control(u_cmd, fix_joint5_angle, x_current, NQ)
            ...
            if enable_collision and not ball_was_hit:
                for ci in range(env.data.ncon):
                    ...
                    if ("ball" in g1 or "ball" in g2) and ("racket" in g1 or "racket" in g2):
                        ...
                        env.set_ball_vel(ball_vel_before_step - (1 + 0.8) * v_rel_n * n_hat)
            ...
            if follow_through_start >= 0:
                ...
                if dt_follow < follow_through_steps:
                    ...
                    u_follow = J_p_f.T @ (200.0 * dp_f - 20.0 * J_p_f @ x_current[NQ:])
        ...
        logger.info("[%2d/%d] seed=%2d %s  err=%.1fmm  type=%s  v_rack=%.2fm/s  (%.0fs)" % (
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=["seed", "hit", "pos_error", "hit_type", "v_racket_at_hit"],
                           extrasaction="ignore")
        w.writeheader()
        w.writerows(results)
    ...
    logger.info("=" * 60)
    logger.info("结果: %d/%d 命中 (%.0f%%)" % (hits, args.n_runs, hits / max(args.n_runs, 1) * 100))
