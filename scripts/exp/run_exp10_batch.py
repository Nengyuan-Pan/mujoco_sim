"""批量运行 exp10_pd_scan 实验（多进程并行）。

用法:
    python scripts/exp/run_exp10_batch.py              # 默认 4 workers
    python scripts/exp/run_exp10_batch.py --workers 8   # 8 workers
"""
import argparse
import os
import subprocess
...
KP_VALUES = [25, 50, 100, 200, 300, 500]
KD_RATIOS = [0.05, 0.1, 0.15, 0.2]
DQ_FRACTIONS = [0.5]
SEEDS = list(range(10))


def run_one(args: tuple[float, float, float, int]) -> tuple[str, bool]:
    """在子进程中运行单次实验。"""
    kp, kdr, dq, seed = args
    tag = f"kp{int(kp)}_kdr{kdr}_dq{dq}_seed{seed}"
    log_path = RAW_DIR / f"{tag}.log"
    if log_path.exists():
        return tag, True

    cmd = [PYTHON_EXE, str(WRAPPER), str(kp), str(kdr), str(dq), str(seed)]
    try:
...
def main() -> None:
    ...
    total = len(tasks)
    print(f"exp10_pd_scan: {len(KP_VALUES)} Kp × {len(KD_RATIOS)} Kd_ratio "
          f"× {len(DQ_FRACTIONS)} dq_frac × {len(SEEDS)} seeds = {total} runs")
