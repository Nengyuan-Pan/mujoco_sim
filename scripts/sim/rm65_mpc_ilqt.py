"""RM-65 双臂机器人 MPC+iLQT 网球击打脚本。

功能：
1. 使用 RM-65 双臂模型（仅驱动右臂 6-DOF）
2. MPC 循环：分层策略（远距雅可比 / 中距单次迭代 / 近距多次迭代）
3. 球和机械臂均由 MuJoCo 物理引擎驱动
4. 击打后球拍-球碰撞使球飞出（物理碰撞效果）
5. 计算完成后以真实速度回放可视化

用法：
  python scripts/rm65_mpc_ilqt.py                  # 默认参数
  python scripts/rm65_mpc_ilqt.py --viewer          # + 真实速度回放
  python scripts/rm65_mpc_ilqt.py --seed 42         # 指定随机种子
  python scripts/rm65_mpc_ilqt.py --fd              # 有限差分线性化
  python scripts/rm65_mpc_ilqt.py --horizon 50      # 自定义 horizon
"""
...
def main() -> None:
    ...
    # 计算球拍中心初始位置，以球拍为基准设置击打点偏移（±0.5m 范围）
    env.reset(init_q)
    ...
    hit_offset_ranges = {
        "x": [racket_offset[0] - 0.20, racket_offset[0] + 0.20],
        "y": [racket_offset[1] - 0.20, racket_offset[1] + 0.20],
        "z": [racket_offset[2] - 0.15, racket_offset[2] + 0.15],
    }

    hit_time = total_horizon * dt * rng.uniform(0.60, 0.90)
    ...
    # 初始雅可比转置控制
    U_prev = compute_jacobian_init_control(env, x0, p_hit, k_hit_total, gain=30.0, fix_joint5_angle=fix_joint5_angle)
    ...
    for step in range(total_horizon):
        ...
        if need_replan:
            ...
            if k_hit_new > far_threshold:
                ...
                u_jt = compute_jacobian_init_control(
                    env, x_current, p_hit_new, replan_interval, gain=60.0,
                ...
                if len(U_prev) >= horizon_plan // 2:
                    ...
                    U_warm = compute_jacobian_init_control(
                        env, x_current, p_hit_new, horizon_plan, gain=30.0,
                        fix_joint5_angle=fix_joint5_angle,
                    )
        ...
        # MPC 阶段禁用球拍碰撞（仅当接近击打时恢复，让球拍物理击球）
        enable_collision = (k_hit_new <= 2)
    ...
    logger.info(f"击打后球速: {ball_speed_after:.2f} m/s")
