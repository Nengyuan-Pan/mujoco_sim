...
def do_replan(
    ...
    cost_fn_plan = HittingCost(
        env_plan, p_follow_new, cfg["v_hit_desired"], Q_p_mat, Q_v_mat, R_mat,
...
def main() -> None:
    ...
    # v4: 叠加 v4 随挥管道配置
    v4_config_path = base_path / "v4_follow_through.yaml"
    if v4_config_path.exists():
        v4_config = load_config(v4_config_path)
        config_dict = merge_configs(config_dict, v4_config)
    ...
    logger.info(f"[v4] 随挥方向(来球反方向): {np.round(d_follow, 3)}")
    logger.info(f"[v4] 击球时刻期望速度: {racket_speed:.1f} m/s")
    logger.info(f"[v4] 终端目标点: {np.round(p_follow, 3)}, 终端速度: {np.linalg.norm(v_hit_desired):.1f} m/s")
    logger.info(f"[v4] 随挥距离: {follow_through_length:.2f}m, 随挥步数: {follow_through_steps}")
    ...
    logger.info(f"[v4] Q_v 方向对齐: scalar={Q_v_scalar}, d_follow={np.round(d_follow, 3)}")
    ...
    for step in range(total_horizon):
        ...
        if ball_was_hit and (step - hit_step) >= 5:
            logger.info(f"步 {step}: 碰撞完成，进入随挥阶段")
