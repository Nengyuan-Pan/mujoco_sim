"""RM-65 双臂机器人 MPC+iLQT 网球击打脚本（关节速度/加速度限位版）。

在 rm65_evaluate.py 基础上新增关节运动约束：
- 关节速度限制：J1-J2 180°/s, J3-J6 225°/s（RM65-B 额定值）
- 关节加速度限制：600°/s²（RM65-B 额定值）
- 末端线速度/加速度限制（软约束惩罚）
- MPC 循环中硬裁剪关节速度

约束实现：
1. 代价函数添加 qdot 惩罚（软约束，引导优化器避开过速区域）
2. MPC 步进后裁剪 qdot 超限（硬约束，确保物理可行性）
3. 控制力矩反算：从加速度限制反推最大允许力矩

用法：
  python scripts/rm65_joint_limit.py --viewer --seed 42
  python scripts/rm65_joint_limit.py --bounce --normal-flip --viewer
"""
...
class JointLimitHittingCost(HittingCost):
    """带关节速度/加速度软约束惩罚的 HittingCost 子类。

    在父类基础上添加：
    - 关节速度惩罚：0.5 * Q_qdot * max(|qdot| - vel_limit, 0)^2
    - 关节加速度惩罚：0.5 * Q_qacc * max(|qacc| - acc_limit, 0)^2
    不修改 src/ilqt/cost.py，仅在此脚本中使用。
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._Q_qdot = Q_qdot
        self._Q_qacc = Q_qacc
        self._vel_limit = vel_limit if vel_limit is not None else RM65_JOINT_VEL_LIMIT
        self._acc_limit = acc_limit if acc_limit is not None else RM65_JOINT_ACC_LIMIT
        self._dt = dt
        self._prev_qdot: np.ndarray | None = None
    ...
    def running_cost(self, x: np.ndarray, u: np.ndarray, k: int | None = None) -> float:
        ...
        # 关节加速度软约束惩罚（用有限差分近似加速度）
        if self._Q_qacc > 0 and self._prev_qdot is not None:
            qacc = (qdot - self._prev_qdot) / self._dt
            excess = np.maximum(np.abs(qacc) - self._acc_limit, 0.0)
            cost += 0.5 * self._Q_qacc * float(excess @ excess)
    ...
    def running_derivatives(
        ...
        if self._Q_qacc > 0 and self._prev_qdot is not None:
            qacc = (qdot - self._prev_qdot) / self._dt
            excess = np.maximum(np.abs(qacc) - self._acc_limit, 0.0)
            sign = np.sign(qacc) / self._dt
            l_x[NQ:] += self._Q_qacc * sign * excess
            for j in range(self.env.NU):
                if excess[j] > 0:
                    l_xx[NQ + j, NQ + j] += self._Q_qacc / (self._dt ** 2)
        self._prev_qdot = qdot.copy()
        return l_x, l_u, l_xx, l_ux, l_uu

    def reset_prev_state(self) -> None:
        """重置前一步状态（MPC 每次重规划时调用）。"""
        self._prev_qdot = None


def clip_joint_velocity(q: np.ndarray, qdot: np.ndarray, vel_limit: np.ndarray) -> np.ndarray:
    """裁剪关节速度到额定限速内。

    如果 |qdot_j| > vel_limit_j，则钳位到 ±vel_limit_j。

    Args:
        q: 关节位置，形状 (6,)（未使用，保留接口一致性）。
        qdot: 关节速度，形状 (6,)。
        vel_limit: 各关节速度限位，形状 (6,)。
...
def clip_joint_acceleration(
    ...
    根据加速度限制反算当前步最大允许速度变化量，
    将 qdot 钳位到 [qdot_prev - acc_limit*dt, qdot_prev + acc_limit*dt]。

    Args:
        qdot: 当前关节速度，形状 (6,)。
        qdot_prev: 上一步关节速度，形状 (6,)。
        acc_limit: 关节加速度限位（rad/s²）。
        dt: 时间步长（秒）。

    Returns:
        裁剪后的关节速度，形状 (6,)。
    """
    max_delta = acc_limit * dt
    delta = qdot - qdot_prev
    delta_clipped = np.clip(delta, -max_delta, max_delta)
    return qdot_prev + delta_clipped
...
# RM-65 关节限位（度），来自 rm65_model.xml 的 range 属性
RM65_JOINT_NAMES = ["shoulder_pan", "shoulder_lift", "elbow", "wrist_1", "wrist_2", "wrist_3"]
RM65_JOINT_LIMIT_DEG = np.array([
    [-178, 178],   # joint1 shoulder_pan
    [-130, 130],   # joint2 shoulder_lift
    [-135, 135],   # joint3 elbow
    [-178, 178],   # joint4 wrist_1
    [-128, 128],   # joint5 wrist_2
    [-360, 360],   # joint6 wrist_3
])
RM65_JOINT_LIMIT_RAD = np.deg2rad(RM65_JOINT_LIMIT_DEG)
# RM-65 额定关节速度（rad/s）：J1-J2=180°/s, J3-J6=225°/s（RM65-B datasheet）
RM65_JOINT_VEL_LIMIT_DEG = np.array([180.0, 180.0, 225.0, 225.0, 225.0, 225.0])
RM65_JOINT_VEL_LIMIT = np.deg2rad(RM65_JOINT_VEL_LIMIT_DEG)
# RM-65 额定关节加速度（rad/s²）：600°/s²（RM65-B datasheet）
RM65_JOINT_ACC_LIMIT_DEG = 600.0
RM65_JOINT_ACC_LIMIT = np.deg2rad(RM65_JOINT_ACC_LIMIT_DEG)
# RM-65 力矩限位（Nm），来自 rm65_model.xml 的 ctrlrange
RM65_TORQUE_LIMIT = np.array([60.0, 60.0, 30.0, 10.0, 10.0, 10.0])
# RM-65 末端默认限速（来自睿尔曼开发者文档）
RM65_TCP_VEL_LIMIT = 0.250       # m/s
RM65_TCP_ACC_LIMIT = 1.600       # m/s²
RM65_TCP_ANGVEL_LIMIT = 0.600    # rad/s
RM65_TCP_ANGACC_LIMIT = 4.000    # rad/s²
...
def evaluate_trajectory(
    ...
    ee_vel = np.zeros((N + 1, 3))
    for k in range(N + 1):
        env.set_arm_state(X[k])
        ee_pos[k] = env.get_ee_pos()
        ee_vel[k] = env.get_ee_vel()
    ee_speed = np.linalg.norm(ee_vel, axis=1)

    # 关节裕度分析
    joint_margins = np.zeros((N + 1, NQ))
    for j in range(NQ):
        lo, hi = RM65_JOINT_LIMIT_RAD[j]
        margin_lo = q_traj[:, j] - lo
        margin_hi = hi - q_traj[:, j]
        joint_margins[:, j] = np.minimum(margin_lo, margin_hi)
    min_margin_per_joint = joint_margins.min(axis=0)
    min_margin_rad = joint_margins.min()
    min_margin_deg = np.rad2deg(min_margin_rad)

    # 关节速度峰值
    peak_qdot = np.abs(qdot_traj).max(axis=0)
    peak_qdot_ratio = peak_qdot / RM65_JOINT_VEL_LIMIT

    # 关节加速度（有限差分）
    qacc_traj = np.zeros_like(qdot_traj)
    qacc_traj[1:] = (qdot_traj[1:] - qdot_traj[:-1]) / dt
    peak_qacc = np.abs(qacc_traj).max(axis=0)
    peak_qacc_ratio = peak_qacc / RM65_JOINT_ACC_LIMIT

    # 力矩峰值
    peak_torque = np.abs(U).max(axis=0)
    peak_torque_ratio = peak_torque / RM65_TORQUE_LIMIT

    # 球拍速度峰值及时间
    peak_ee_speed = ee_speed.max()
    peak_ee_speed_step = ee_speed.argmax()
    peak_ee_speed_time = peak_ee_speed_step * dt

    # 击打时刻球拍速度
    if hit_step >= 0 and hit_step < len(ee_speed):
        ee_speed_at_hit = ee_speed[hit_step]
        ee_vel_at_hit = ee_vel[hit_step]
    else:
        ee_speed_at_hit = ee_speed[-1]
        ee_vel_at_hit = ee_vel[-1]

    # 位置误差时间序列
    pos_err_traj = np.linalg.norm(ee_pos - p_hit, axis=1)

    return {
        "t_arr": t_arr, "t_ctrl": t_ctrl,
        "q_traj": q_traj, "qdot_traj": qdot_traj, "U": U,
        "ee_pos": ee_pos, "ee_vel": ee_vel, "ee_speed": ee_speed,
        "ball_pos": ball_pos_arr[:N + 1],
        "pos_err_traj": pos_err_traj,
        "joint_margins": joint_margins,
        "min_margin_per_joint": min_margin_per_joint,
        "min_margin_rad": min_margin_rad,
        "min_margin_deg": min_margin_deg,
        "peak_qdot": peak_qdot,
        "peak_qdot_ratio": peak_qdot_ratio,
        "qacc_traj": qacc_traj,
        "peak_qacc": peak_qacc,
        "peak_qacc_ratio": peak_qacc_ratio,
        "peak_torque": peak_torque,
        "peak_torque_ratio": peak_torque_ratio,
        "peak_ee_speed": peak_ee_speed,
        "peak_ee_speed_time": peak_ee_speed_time,
        "ee_speed_at_hit": ee_speed_at_hit,
        "ee_vel_at_hit": ee_vel_at_hit,
        "hit_step": hit_step,
        "v_hit_desired": v_hit_desired,
    }


def plot_evaluation(eval_data: dict, save_path: str) -> None:
    """绘制评估报告（6个子图）。

    Args:
        eval_data: evaluate_trajectory 返回的指标字典。
    ...
    t = eval_data["t_arr"]
    t_ctrl = eval_data["t_ctrl"]
    hit_step = eval_data["hit_step"]
    hit_time = hit_step * (t[1] - t[0]) if hit_step >= 0 else None

    fig, axes = plt.subplots(3, 2, figsize=(16, 14))
    fig.suptitle("RM-65 MPC Tennis Hitting Evaluation", fontsize=16, fontweight="bold")

    colors = plt.cm.tab10(np.linspace(0, 1, 6))

    # 1. 球拍中心速度
    ax = axes[0, 0]
    ax.plot(t, eval_data["ee_speed"], "b-", linewidth=1.2, label="Racket speed")
    if hit_time is not None:
        ax.axvline(hit_time, color="r", linestyle="--", alpha=0.7, label="Hit")
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Speed (m/s)")
    ax.set_title(f"Racket Center Speed (peak={eval_data['peak_ee_speed']:.2f} m/s)")
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3)

    # 2. 位置误差
    ax = axes[0, 1]
    ax.plot(t, eval_data["pos_err_traj"] * 100, "r-", linewidth=1.2)
    if hit_time is not None:
        ax.axvline(hit_time, color="r", linestyle="--", alpha=0.7, label="Hit")
    ax.axhline(5.0, color="g", linestyle=":", alpha=0.5, label="5cm")
    ax.axhline(10.0, color="orange", linestyle=":", alpha=0.5, label="10cm")
    ax.set_xlabel("Time (s)")
    ...
    ax = axes[1, 0]
    for j in range(6):
        lo_deg = RM65_JOINT_LIMIT_DEG[j, 0]
        hi_deg = RM65_JOINT_LIMIT_DEG[j, 1]
        q_deg = np.rad2deg(eval_data["q_traj"][:, j])
        ax.plot(t, q_deg, color=colors[j], linewidth=1.0, label=f"J{j+1}")
        ax.axhline(hi_deg, color=colors[j], linestyle=":", alpha=0.3)
        ax.axhline(lo_deg, color=colors[j], linestyle=":", alpha=0.3)
    if hit_time is not None:
        ax.axvline(hit_time, color="r", linestyle="--", alpha=0.7)
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Joint Angle (deg)")
    ax.set_title("Joint Angles vs Limits (dashed=limits)")
    ...
    ax = axes[1, 1]
    for j in range(6):
        ax.plot(t, np.rad2deg(eval_data["qdot_traj"][:, j]),
                color=colors[j], linewidth=1.0, label=f"J{j+1}")
        vel_limit_deg = np.rad2deg(RM65_JOINT_VEL_LIMIT[j])
        ax.axhline(vel_limit_deg, color=colors[j], linestyle=":", alpha=0.3)
        ax.axhline(-vel_limit_deg, color=colors[j], linestyle=":", alpha=0.3)
    if hit_time is not None:
        ax.axvline(hit_time, color="r", linestyle="--", alpha=0.7)
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Joint Velocity (deg/s)")
    ax.set_title("Joint Velocities vs Rated Limits (dashed=limits)")
    ax.legend(fontsize=7, ncol=3)
    ax.grid(True, alpha=0.3)
    ...
    ax = axes[2, 0]
    for j in range(6):
        ax.plot(t_ctrl, eval_data["U"][:, j],
                color=colors[j], linewidth=1.0, label=f"J{j+1}")
        ax.axhline(RM65_TORQUE_LIMIT[j], color=colors[j], linestyle=":", alpha=0.3)
        ax.axhline(-RM65_TORQUE_LIMIT[j], color=colors[j], linestyle=":", alpha=0.3)
    if hit_time is not None:
        ax.axvline(hit_time, color="r", linestyle="--", alpha=0.7)
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Torque (Nm)")
    ax.set_title("Joint Torques vs Limits (dashed=limits)")
    ax.legend(fontsize=7, ncol=3)
    ax.grid(True, alpha=0.3)
    ...
    ax = axes[2, 1]
    for j in range(6):
        ax.plot(t, np.rad2deg(eval_data["joint_margins"][:, j]),
                color=colors[j], linewidth=1.0, label=f"J{j+1}")
    if hit_time is not None:
        ax.axvline(hit_time, color="r", linestyle="--", alpha=0.7)
    ax.axhline(0, color="k", linestyle="-", alpha=0.5)
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Limit Margin (deg)")
    ax.set_title("Joint Limit Margins (0=at limit, negative=exceeded)")
    ax.legend(fontsize=7, ncol=3)
    ax.grid(True, alpha=0.3)
    ...
    plt.close()
    logger.info(f"评估报告已保存: {save_file}")


def print_evaluation_report(eval_data: dict, pos_error: float, vel_error: float,
                            ball_speed_before: float, ball_speed_after: float) -> None:
    """打印详细评估报告。"""
    print("\n" + "=" * 60)
    print("  RM-65 Tennis Hitting Evaluation Report")
    print("=" * 60)
    print(f"\n--- Hit Accuracy ---")
    print(f"  Position error: {pos_error * 100:.2f} cm")
    print(f"  Velocity error: {vel_error:.2f} m/s")
    print(f"\n--- Ball Speed ---")
    print(f"  Before hit: {ball_speed_before:.2f} m/s")
    print(f"  After hit:  {ball_speed_after:.2f} m/s")
    speed_change = ball_speed_after - ball_speed_before
    print(f"  Change:     {speed_change:+.2f} m/s")
    if ball_speed_before > 0:
        print(f"  Rebound ratio: {ball_speed_after / ball_speed_before:.2f}x")
    print(f"\n--- Racket Center Speed ---")
    print(f"  At hit time: {eval_data['ee_speed_at_hit']:.2f} m/s")
    print(f"  Direction:   [{eval_data['ee_vel_at_hit'][0]:.2f}, {eval_data['ee_vel_at_hit'][1]:.2f}, {eval_data['ee_vel_at_hit'][2]:.2f}]")
    print(f"  Desired vel: [{eval_data['v_hit_desired'][0]:.2f}, {eval_data['v_hit_desired'][1]:.2f}, {eval_data['v_hit_desired'][2]:.2f}]")
    print(f"  Peak speed:  {eval_data['peak_ee_speed']:.2f} m/s (t={eval_data['peak_ee_speed_time']:.3f}s)")
    print(f"\n--- Joint Limit Analysis ---")
    for j in range(6):
        margin_deg = np.rad2deg(eval_data["min_margin_per_joint"][j])
        lo_deg = RM65_JOINT_LIMIT_DEG[j, 0]
        hi_deg = RM65_JOINT_LIMIT_DEG[j, 1]
        q_min = np.rad2deg(eval_data["q_traj"][:, j].min())
        q_max = np.rad2deg(eval_data["q_traj"][:, j].max())
        status = "OK" if margin_deg > 5 else ("WARN" if margin_deg > 0 else "EXCEED!")
        print(f"  J{j+1} ({RM65_JOINT_NAMES[j]:14s}): limit=[{lo_deg:>4d},{hi_deg:>4d}]deg, "
              f"actual=[{q_min:>7.1f},{q_max:>7.1f}]deg, margin={margin_deg:>6.1f}deg  {status}")
    print(f"  Global min margin: {eval_data['min_margin_deg']:.1f} deg")
    print(f"\n--- Joint Velocity Analysis ---")
    for j in range(6):
        peak = np.rad2deg(eval_data["peak_qdot"][j])
        limit = np.rad2deg(RM65_JOINT_VEL_LIMIT[j])
        ratio = eval_data["peak_qdot_ratio"][j]
        status = "OK" if ratio < 0.9 else ("WARN" if ratio < 1.0 else "EXCEED!")
        print(f"  J{j+1} ({RM65_JOINT_NAMES[j]:14s}): peak={peak:>7.1f} deg/s, limit={limit:>5.1f} deg/s, "
              f"ratio={ratio:.1%}  {status}")
    print(f"\n--- Joint Acceleration Analysis ---")
    for j in range(6):
        peak = np.rad2deg(eval_data["peak_qacc"][j])
        limit = np.rad2deg(RM65_JOINT_ACC_LIMIT)
        ratio = eval_data["peak_qacc_ratio"][j]
        status = "OK" if ratio < 0.9 else ("WARN" if ratio < 1.0 else "EXCEED!")
        print(f"  J{j+1} ({RM65_JOINT_NAMES[j]:14s}): peak={peak:>7.1f} deg/s², limit={limit:>5.1f} deg/s², "
              f"ratio={ratio:.1%}  {status}")
    print(f"\n--- Torque Analysis ---")
    for j in range(6):
        peak = eval_data["peak_torque"][j]
        limit = RM65_TORQUE_LIMIT[j]
        ratio = eval_data["peak_torque_ratio"][j]
        status = "OK" if ratio < 0.9 else ("WARN" if ratio < 1.0 else "CLIPPED!")
        print(f"  J{j+1} ({RM65_JOINT_NAMES[j]:14s}): peak={peak:>7.2f} Nm, limit={limit:>5.1f} Nm, "
              f"ratio={ratio:.1%}  {status}")
    print("=" * 60 + "\n")
...
def main() -> None:
    """RM-65 MPC 主函数。"""
    parser = argparse.ArgumentParser(description="RM-65 MPC+iLQT 网球击打评估")
    ...
    parser.add_argument("--bounce", action="store_true", help="启用反弹球模式：球从约20m处飞来，地面弹跳一次后到达击打区")
    parser.add_argument("--bounce-distance", type=float, default=20.0, help="反弹球发球距离 (m)，默认20")
    parser.add_argument("--bounce-speed-min", type=float, default=16.0, help="反弹球到达击打区最低速度 (m/s)，默认16")
    parser.add_argument("--bounce-speed-max", type=float, default=22.0, help="反弹球到达击打区最高速度 (m/s)，默认22")
    ...
    parser.add_argument("--no-plot", action="store_true", help="不生成 matplotlib 评估图")
    parser.add_argument("--clip-vel", action="store_true", help="启用关节速度硬裁剪（额定180/225°/s）")
    parser.add_argument("--clip-acc", action="store_true", help="启用关节加速度硬裁剪（额定600°/s²），需配合 --clip-vel")
    parser.add_argument("--qdot-weight", type=float, default=0.0, help="关节速度软约束惩罚权重（0=禁用，建议100~10000）")
    parser.add_argument("--qacc-weight", type=float, default=0.0, help="关节加速度软约束惩罚权重（0=禁用，建议10~1000）")
    parser.add_argument("--racket-speed", type=float, default=2.5, help="限速模式下的期望球拍速度 (m/s)，默认2.5（额定限速下合理值）")
    parser.add_argument("--long-horizon", action="store_true", help="增加仿真步数到400步（2秒），给限速下更多挥拍时间")
    ...
    # RM-65 专用 MPC 参数
    # 两阶段策略：阶段1 仅位置代价（快速收敛），阶段2 完整代价（精细速度）
    mpc_cfg = config.get("mpc", {})
    total_horizon = 200
    if args.long_horizon or args.clip_vel:
        total_horizon = 400
    if args.bounce:
        total_horizon = max(total_horizon, int(args.bounce_distance / 20.0 * 240))
    fixed_horizon = 30
    replan_interval = 10
    max_iter_per_plan = 8
    Q_p_scale_far = 5.0
    ...
    # ===== 关节速度硬限制：保存限速参数，在 MPC 循环中裁剪 =====
    joint_vel_limit = RM65_JOINT_VEL_LIMIT if args.clip_vel else None
    joint_acc_limit = RM65_JOINT_ACC_LIMIT if args.clip_acc else None
    if args.clip_vel:
        logger.info(f"关节速度硬限制已启用: {np.rad2deg(RM65_JOINT_VEL_LIMIT).astype(int)} deg/s")
    if args.clip_acc:
        logger.info(f"关节加速度硬限制已启用: {RM65_JOINT_ACC_LIMIT_DEG:.0f} deg/s²")
    ...
    if args.bounce:
        # 反弹球模式：球从约20m飞来，地面弹跳一次
        # 球速~20m/s，20m需要~1.2s，击打在仿真后期
        hit_time = total_horizon * dt * rng.uniform(0.65, 0.80)
        # 击打区域与直球一致：target_center ± target_offset
        hit_offset_ranges = {
            "x": [target_center[0] - target_offset - shoulder_pos[0],
                   target_center[0] + target_offset - shoulder_pos[0]],
            "y": [target_center[1] - target_offset - shoulder_pos[1],
                   target_center[1] + target_offset - shoulder_pos[1]],
            "z": [target_center[2] - target_offset - shoulder_pos[2],
                   target_center[2] + target_offset - shoulder_pos[2]],
        }
        p0, v0, p_hit_expected = generate_bounce_ball(
            shoulder_pos, workspace_radius, hit_time, g,
            serve_distance=args.bounce_distance,
            bounce_restitution=0.75,
            target_speed_range=(args.bounce_speed_min, args.bounce_speed_max),
            hit_offset_ranges=hit_offset_ranges,
            rng=rng,
            ball_direction="y",
        )
        logger.info(f"反弹球模式: 发球距离={args.bounce_distance}m, 目标速度=[{args.bounce_speed_min}, {args.bounce_speed_max}] m/s")
    else:
    ...
    # 限速模式下使用更低的期望球拍速度
    if args.racket_speed is not None:
        racket_speed = args.racket_speed
        logger.info(f"限速模式球拍速度: {racket_speed:.1f} m/s")
    ...
    cost_fn = JointLimitHittingCost(
        ...
        Q_qacc=args.qacc_weight,
        vel_limit=RM65_JOINT_VEL_LIMIT,
        acc_limit=RM65_JOINT_ACC_LIMIT,
        dt=dt,
    )
    ...
    for step in range(total_horizon):
        ...
        if need_replan:
            ...
            if k_hit_new > far_threshold:
                ...
                if hasattr(cost_fn, "reset_prev_state"):
                    cost_fn.reset_prev_state()
                ...
                if use_r_decay:
                    R_schedule_new = compute_r_schedule(
                        horizon_plan, R, decay_ratio=r_decay_ratio,
                    )
                ...
                if use_backswing:
                    ...
                    backswing_scale = horizon_plan / max(k_hit_total, 1)
                    q_des_traj_new = np.zeros((horizon_plan, env.NQ))
                    q_des_traj_new[:, 0] = compute_joint1_backswing_trajectory(
                    ...
                    for j in range(1, env.NQ):
                        q_des_traj_new[:, j] = np.linspace(x_current[j], q_hit_new_ik[j], horizon_plan)

                    cost_fn.set_q_des_traj(q_des_traj_new, Q_joint=Q_joint)

                    # 用后摆轨迹更新 Warm-start
                    if len(U_prev) >= horizon_plan // 2:
                        ...
                        U_warm, _ = generate_backswing_warm_start(
                            env, x_current, p_follow_new, v_hit_desired, horizon_plan,
                            backswing_offset=backswing_offset * backswing_scale,
                            backswing_ratio=backswing_ratio,
                            fix_joint5_angle=fix_joint5_angle,
                            n_des=n_des_new,
                    ...
                    if len(U_prev) >= horizon_plan // 2:
                        ...
                        U_warm = compute_jacobian_init_control(
                            env, x_current, p_follow_new, horizon_plan, gain=30.0,
                            fix_joint5_angle=fix_joint5_angle,
                        )
        ...
        if joint_vel_limit is not None:
            qdot_raw = x_current[env.NQ:]
            qdot_clipped = clip_joint_velocity(x_current[:env.NQ], qdot_raw, joint_vel_limit)
            if joint_acc_limit is not None:
                qdot_clipped = clip_joint_acceleration(qdot_clipped, qdot_before, joint_acc_limit, dt)
    ...
    eval_data = evaluate_trajectory(
        ...
        hit_step=hit_step, p_hit=p_hit,
        v_hit_desired=v_hit_desired, dt=dt,
    )

    print_evaluation_report(
        eval_data, pos_error, vel_error,
        speed_before, ball_speed_after,
    )
    ...
    if not args.no_plot:
        ...
        save_path = str(results_dir / "rm65_evaluation")
        plot_evaluation(eval_data, save_path)
