"""RM-65 双臂机器人 MPC+iLQR 网球击打（快速版）。

在 rm65_mpc_ilqr_5_5.py 基础上优化：
- 减少重规划频率（replan_interval=20，节省 ~50% 计算）
- 降低 iLQR 迭代次数（首次=15，远距=5，近距=8，规划地平线封顶 40 步）
- 首次保留线搜索保稳定，其余跳线搜索加速
- 后摆轨迹用完整地平线生成后截断，保证质量
- 目标：冲击实时比率 1.0x
...
def main() -> None:
    ...
    Q_v_scale_near = 50.0
    # 快速版：迭代次数控制
    first_plan_iters = 15             # 首次迭代6次
    near_plan_iters = 8                # 近距迭代2次
